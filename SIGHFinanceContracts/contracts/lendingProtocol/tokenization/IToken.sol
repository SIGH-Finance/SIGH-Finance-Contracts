 ...
 */
contract IToken is VersionedInitializable, IncentivizedERC20, IIToken {
...
//  ###############################################################
//  ######### FUNCTIONS CALLABLE ONLY BY THE LENDING POOL #########
//  ###############################################################
   ...
   * @param index The new liquidity index of the reserve
   */
  function mintToTreasury(uint256 amount, address sighPayAggregator, uint256 index) external override onlyLendingPool {
    if (amount == 0) {
      return;
    }
    ...
    _mint(sighPayAggregator, amount.rayDiv(index));

    emit Transfer(address(0), sighPayAggregator, amount);
    emit Mint(sighPayAggregator, amount, index);
  }
...
//  ######################################
  ...
  /**
   * @dev Transfers the ITokens between two users. Validates the transfer
